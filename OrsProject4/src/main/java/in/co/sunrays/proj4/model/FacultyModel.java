	...
	public void update(FacultyBean bean) throws ApplicationException, DuplicateRecordException {
	   ...
	   CourseModel courseModel = new CourseModel();	
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn.prepareStatement("UPDATE ST_FACULTY SET COLLEGE_ID=?, COLLEGE_NAME=?, COURSE_ID=?, COURSE_NAME=?, SUBJECT_ID=?, SUBJECT_NAME=?, FIRST_NAME=?, LAST_NAME=?, QUALIFICATION=?, EMAIL_ID=?, DOB=?, MOBILE_NO=?, CREATED_BY=?, MODIFIED_BY=?, CREATED_DATETIME=?, MODIFIED_DATETIME=?, GENDER=? WHERE ID=?");
			...
			pstmt.setString(17, bean.getGender());
